...
public class SerialModule {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case FP_GetImage:
                    ...
                    Log.d("WorkerHandler", "valueGetImage222222222:" + valueGetImage);
                ...
                case FP_GenChar:
                    ...
                    mMsgThreadHandler.obtainMessage(FP_GenChar, valueGenChar, msg.arg1)
                            .sendToTarget();
                    break;
                case FP_RegModel:
                ...
                case FP_Match:
                    ...
                    mMsgThreadHandler.obtainMessage(FP_Match, Boolean.valueOf(valueMatch))
                            .sendToTarget();
                    break;
                case FP_StoreChar:
                    int valueStoreChar = PSStoreChar(msg.arg1, msg.arg2);
                    mMsgThreadHandler.obtainMessage(FP_StoreChar, valueStoreChar, -1)
                            .sendToTarget();
                    break;
                case FP_LoadChar:
                ...
                case FP_DeleteChar:
                    ...
                    mMsgThreadHandler.obtainMessage(FP_DeleteChar, valueDeleteChar, -1)
                            .sendToTarget();
                    break;
                case FP_Empty:
                ...
                case FP_GetImageEX:
                    ...
                    mMsgThreadHandler.obtainMessage(FP_GetImageEX, valueGetImageEx, -1)
                            .sendToTarget();
                    break;
                case FP_UpImageEX:
                ...
                case FP_GenCharEX:
                    ...
                    mMsgThreadHandler.obtainMessage(FP_GenCharEX, valueGenCharEx, msg.arg1)
                            .sendToTarget();
                    break;
                default:
                    break;
    ...
    private synchronized boolean PSMatch() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte)
                0xff, (byte) 0x01, (byte) 0x00, (byte) 0x03, (byte) 0x03, (byte) 0x00, (byte) 0x07};
        sendCommand(command);
    ...
    private synchronized byte PSDeleteChar(short pageIDStart, short delNum) {
        ...
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte)
                ...
                pageIDArray[1], delNumArray[0], delNumArray[1], checkSumArray[0], checkSumArray[1]};
        sendCommand(command);
        int length = read(buffer, 12, 100);
    ...
    private synchronized byte PSEmpty() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte)
                0xff, (byte) 0x01, (byte) 0x00, (byte) 0x03, (byte) 0x0d, (byte) 0x00, (byte) 0x11};
        sendCommand(command);
        int length = read(buffer, 12, 100);
    ...
    private synchronized byte[] PSUpImage() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte)
                0xff, (byte) 0x01, (byte) 0x00, (byte) 0x03, (byte) 0x0a, (byte) 0x00, (byte) 0x0e};
        sendCommand(command);
        if (buffer.length == 61440) {
            Log.d("SerialModule", "buffer.length:" + buffer.length);
            int length = read(buffer, UP_IMAGE_RESPONSE_SIZE, 300);
    ...
    private synchronized byte[] PSUpImageEx() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte)
                0xff, (byte) 0x01, (byte) 0x00, (byte) 0x03, (byte) 0x31, (byte) 0x00, (byte) 0x35};
        sendCommand(command);
    ...
    public void openSerialPort(String uartName, int baudRate, boolean bSPI) throws
    ...
    private boolean checkCmdTag(byte[] data) {
        for (int i = 0; i < data.length - 4; ) {
            if (((byte) data[i] == (byte) (0xEF)) && ((byte) data[i + 1] == (byte) (0x01)) && (
                    (byte) data[i + 2] == (byte) (0xFF)) && ((byte) data[i + 3] == (byte) (0xFF))) {
                return true;
            }
            break;
        }
        ...
        if (!mIsSPI) {
            ...
            if (mCurrentSize > 0) {
                while (!shutDown) {
                    ...
                    if (readDataLength[0] == readDataLength[1] && readDataLength[1] ==
                            readDataLength[2]) {
                        shutDown = true;
                    }
                }
            ...
            for (int i = 0; i < mCurrentSize - 4; ) {
                ...
                if (((byte) mBuffer[i] == (byte) (0xEF)) && ((byte) mBuffer[i + 1] == (byte)
                        (0x01)) && ((byte) mBuffer[i + 2] == (byte) (0xFF)) && ((byte) mBuffer[i
                        + 3] == (byte) (0xFF))) {
                    int pkgsize = (int) (mBuffer[i + 8]) + ((int) (mBuffer[i + 7] << 8) & 0xFF00)
                            + 9;
                    if (pkgsize == -117) {
                        pkgsize = 139;
                    }
