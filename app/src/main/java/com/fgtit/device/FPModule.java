...
public class FPModule {
    ...
    public FPModule() {
        ...
        Log.i("xpb", "Device Info=" + String.valueOf(mDeviceType) + "/" + String.valueOf
				(mDeviceIO) + "/" + String.valueOf(mBaudRate) + "/" + mUartName);
    }
    ...
    public int getDeviceType() {
        ...
        //8'
        if (devname.equals("FP08") || devname.equals("FP-08") || devname.equals("FP-08T") ||
				devname.equals("TIQ-805Q")) {
        ...
        if (devname.equals("mbk82_tb_kk") || devname.equals("iMA122") || devname.equals("iMA321")
				|| devname.equals("iMA322") || devname.equals("BioMatch FM-01") || devname.equals
				("FP05") || devname.equals("FP-05") || devname.equals("KT-7500")) {
    ...
    public void ResumeRegister() {
        mPermissionIntent = PendingIntent.getBroadcast(pContext, 0, new Intent
    ...
    public void PauseUnRegister() {
        if (pContext != null && mUsbReceiver != null) {
            pContext.unregisterReceiver(mUsbReceiver);
        }
    }

    public void requestPermission() {
        UsbManager pmusbManager = (UsbManager) ((Activity) pContext).getSystemService(Context
				.USB_SERVICE);
        ...
        while (deviter.hasNext()) {
            ...
            if ((tmpusbdev.getVendorId() == 0x0453) && (tmpusbdev.getProductId() == 0x9005)) {
                ...
                break;
            } else if ((tmpusbdev.getVendorId() == 0x2009) && (tmpusbdev.getProductId() ==
					0x7638)) {
                Log.i("xpb", "usb=0x2009,0x7638");
                pmusbDevice = tmpusbdev;
                break;
            } else if ((tmpusbdev.getVendorId() == 0x2109) && (tmpusbdev.getProductId() ==
					0x7638)) {
                Log.i("xpb", "usb=0x2109,0x7638");
                pmusbDevice = tmpusbdev;
                break;
            } else if ((tmpusbdev.getVendorId() == 0x0483) && (tmpusbdev.getProductId() ==
					0x5720)) {
        ...
        if (pmusbDevice != null) {
            if (!pmusbManager.hasPermission(pmusbDevice)) {
                synchronized (mUsbReceiver) {
                    Log.d("FPModule", "mUsbReceiver:" + mUsbReceiver);
    ...
    public boolean MatchTemplate(byte[] refdat, int refsize, byte[] matdat, int matsize, int
			score) {
        int rc = refsize / 256;
        int mc = matsize / 256;
