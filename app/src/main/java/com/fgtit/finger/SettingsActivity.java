...
public class SettingsActivity extends PreferenceActivity implements OnPreferenceChangeListener {
    ...
    public static final String nevsid = "nevsid";
    ...
    private EditTextPreference mnevsIdEt;
    ...
    private void setupViews() {
        ...
        mnevsIdEt = (EditTextPreference) findPreference(nevsid);
        ...
        mnevsIdEt.setOnPreferenceChangeListener(this);
        ...
        mnevsIdEt.setOnPreferenceClickListener(createOnPreferenceClickListener(mnevsIdEt));
    ...
    private void displayPreference(final Preference preference) {
        ...
        if (preference == mnevsIdEt)
            preference.setSummary(AppPrefs.get().getNevsid());
    }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (preference == mServerIP || preference == mServerPort
                ...
                || preference == mLoginUser || preference == mLoginPassword || preference == mnevsIdEt) {
            preference.setSummary(newValue.toString());
        }
        if (preference == mnevsIdEt) {
            String newNevsid = (String) newValue;
            AppPrefs.get().setNevsid(newNevsid);

            Threading.async(APIClient.getService().getDept(newNevsid).doOnNext(new Consumer<List<DeptsResponse>>() {
                @Override
                public void accept(List<DeptsResponse> deptsResponses) throws Exception {
                    if (deptsResponses.size() != 0) {
                        if (deptsResponses.get(0).school != null) {
                            AppPrefs.get().setSchool(deptsResponses.get(0).school);
                        }
                    }
                    AppPrefs.get().saveFacultyDeps(deptsResponses);
                }
            }));

            Threading.async(APIClient.getService().getStateLgas("").doOnNext(new Consumer<List<StatesResponse>>() {
                @Override
                public void accept(List<StatesResponse> stateResponses) throws Exception {
                    if (stateResponses.size() != 0) {
//                        if (stateResponses.get(0).school != null) {
//                            AppPrefs.get().setSchool(stateResponses.get(0).school);
//                        }
                    }
                    AppPrefs.get().saveLgaStates(stateResponses);
                    AppPrefs.get().getStateLgas();
                }
            }));
